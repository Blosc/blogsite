...
For small arrays that fit snugly into the CPU's high-speed cache, SIMD was quite good in accelerating computations. But as soon as the arrays grew larger, the performance boost vanished. Some of us were already suspicious about the new "memory wall" that was being recently introduced by the widening gap between CPU speeds and memory bandwidth.  However, we couldn't quite put our finger on it.
...
That was my lightbulb moment. For data-intensive tasks, raw CPU clock speed wasn't the whole story. Memory bandwidth was a kingmaker.
...
To establish a baseline, we implemented an out-of-core solution for NumPy/NumExpr leveraging memory-mapped files. The comparison reveals that the overhead from this memory-mapping approach is substantial enough to give Blosc2 a performance edge, particularly for memory-bound operations on compressed data.
...
A notable finding for the AMD system is that Blosc2's on-disk operations are noticeably faster than its in-memory operations, especially for memory-bound tasks. This is likely due to two factors: first, the larger datasets used for on-disk tests allow Blosc2 to use more efficient internal partitions (chunks and blocks), and second, parallel data reads from disk further reduce bandwidth requirements.

In contrast, for CPU-bound operations, on-disk performance is comparable to, albeit slightly slower than, in-memory performance. The analysis also reveals a specific weakness: small matrix multiplications (matmul0) are significantly slower on-disk, identifying a clear target for future optimization.
